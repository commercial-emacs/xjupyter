...
class Spec(object):
    ...
    def response(cls, id: str | int | None, result: Any, /) -> str:
        """
        Bundle serialized *result* into jsonrpc message.
        ...
        try:
            res = ['"jsonrpc":"2.0"']
            res.append('"id":{}'.format(json.dumps(id)))
            res.append('"result":{}'.format(json.dumps(result)))
        except Exception as e:
            raise RPCParseError(str(e))
...
class RPC(object):
    ...
    *client.py*
    ...
    def _handle_request(self, req: dict[str, Any]) -> None:
        """
        Handles an incoming request *req*.
        ...
        try:
            ...
            result = method(req["id"],
                            *req["params"]["args"],
                            **req["params"]["kwargs"])
            if result:
                self._write(Spec.response(req["id"], result))
        except Exception as e:
...
class Watchdog(threading.Thread):
    ...
    def run(self) -> None:
        ...
        while not self._stop.is_set():
            ...
            backoff = 0.005
            while not select.select([self.rpc.stdin, ], [], [], backoff)[0]:
                backoff = min(2*backoff, .075)
                for id, m in self.rpc.target.iopub_msg():
                    backoff = 0.005
                    self.rpc._write(Spec.response(id, m))
                for id, m in self.rpc.target.shell_msg():
                    backoff = 0.005
                    self.rpc._write(Spec.response(id, m))
                for m in self.rpc.target.control_msg():
                    backoff = 0.005
                    self.rpc._write(Spec.response(None, m))
            ...
            if self.rpc.stdin.isatty():
                ...
                try:
                    ...
                    # when stdin is closed externally
                    pass
